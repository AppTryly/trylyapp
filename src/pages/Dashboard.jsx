...
export default function Dashboard({ session }) {
  ...
  useEffect(() => {
    ...
    if (inviteId && inviteId !== session?.user?.id) {
      ...
      supabase.from('profiles').select('*').eq('id', inviteId).single().then(({ data: profile }) => {
        if (profile) setSearchResult(profile);
      });
    }
  ...
  const submitReflection = async () => {
    ...
    if (!nextMissionData) { setTrailJustCompleted(true); setView('feedback'); } else { setView('feedback'); }
  };
  ...
  if (view === 'feedback') {
    ...
    const handleShare = async () => {
      ...
      if (navigator.share) {
        try { await navigator.share({ title: 'Tryly', text, url: window.location.origin }); } catch (e) { if (e.name !== 'AbortError') navigator.clipboard?.writeText(text); }
    ...
    return (
      <div className="container center" style={{justifyContent:'center'}}>
        ...
        {!trailJustCompleted && <button onClick={() => { confettiFired.current = false; backToHome(); }} style={{marginTop: 15}}>Voltar ao Menu</button>}
      </div>
    );
  }
    ...
    <div className="container" style={{position: 'relative'}}>
      <header style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20}}>
        ...
        <div style={{display: 'flex', gap: 10}}>
            <button className="outline" style={{padding: 10, width: 'auto', borderRadius: '50%', borderColor: 'var(--primary)', color: 'var(--primary)'}} onClick={() => { setShowWeeklyModal(true); fetchWeeklyData(); }} title="Sua semana" aria-label="Ver resumo da sua semana"><Calendar size={20}/></button>
            <button className="outline" style={{padding: 10, width: 'auto', borderRadius: '50%', borderColor: 'var(--primary)', color: 'var(--primary)'}} onClick={() => { setShowFriendsModal(true); fetchFriendsData(); }} aria-label="Abrir comunidade e amigos"><UserPlus size={20}/></button>
            {isAdmin && (<button className="outline" style={{padding: 10, width: 'auto', borderRadius: '50%', borderColor: '#7C3AED', color:'#7C3AED'}} onClick={() => navigate('/admin')} aria-label="Painel administrativo"><Settings size={20}/></button>)}
            <div role="button" tabIndex={0} onClick={() => navigate('/profile')} onKeyDown={(e) => e.key === 'Enter' && navigate('/profile')} style={{width: 45, height: 45, borderRadius: '50%', background: '#E2E8F0', overflow: 'hidden', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center'}} aria-label="Abrir meu perfil">{avatarUrl ? <img src={avatarUrl} alt="" style={{width: '100%', height: '100%', objectFit: 'cover'}} /> : <User size={24} color="#64748B"/>}</div>
        </div>
      </header>
...
{!isPro && activeTrail && (
      ...
      <div style={{marginBottom: 40}}>
        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 8}}><div><h3 style={{margin: 0, fontSize: '1.1rem'}}>{activeTrail?.title || 'Carregando...'}</h3><small style={{color: 'var(--text-muted)'}}>{activeTrail?.description}</small></div><span style={{fontWeight: 'bold', color: 'var(--primary)'}}>{percent}%</span></div>
        ...
        <div className="popup-overlay" style={{position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 300, background: 'rgba(0,0,0,0.8)', display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
          <div className="mission-card" style={{width: '90%', maxWidth: '340px', textAlign: 'center', padding: '28px 20px'}}>
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20}}>
              ...
              <button onClick={() => setShowWeeklyModal(false)} style={{width: 'auto', padding: 5, background: 'transparent', color: 'var(--text-muted)', border: 'none'}} aria-label="Fechar modal"><X size={20}/></button>
            </div>
      ...
      {showFriendsModal && (
          <div className="popup-overlay" style={{position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 300, background: 'rgba(0,0,0,0.8)', display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
              <div className="mission-card" style={{width: '90%', maxWidth: '400px', maxHeight: '80vh', display: 'flex', flexDirection: 'column', margin: 0, padding: 0, overflow: 'hidden'}}>
                  <div style={{padding: '15px 20px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}><h3 style={{margin: 0}}>Comunidade</h3><button onClick={() => setShowFriendsModal(false)} style={{width: 'auto', padding: 5, background: 'transparent', color: '#64748B', border: 'none'}} aria-label="Fechar modal"><X size={20}/></button></div>
